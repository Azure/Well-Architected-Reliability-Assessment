...
[Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSUseShouldProcessForStateChangingFunctions','', Justification='This will be fixed in refactor')]
    ...
    if (!$PPTTemplateFile) {
            if ((Test-Path -Path ("$((Get-Location).path)\Azure-Proactive-Resiliency-Library-v2\tools\Mandatory - Executive Summary presentation - Template.pptx") -PathType Leaf) -eq $true) {
                    $PPTTemplateFile = "$((Get-Location).path)\Azure-Proactive-Resiliency-Library-v2\tools\Mandatory - Executive Summary presentation - Template.pptx"
                }
            else {
    ...
    if (!$WordTemplateFile) {
            if ((Test-Path -Path ("$((Get-Location).path)\Azure-Proactive-Resiliency-Library-v2\tools\\Optional - Assessment Report - Template.docx") -PathType Leaf) -eq $true) {
                    $WordTemplateFile = "$((Get-Location).path)\Azure-Proactive-Resiliency-Library-v2\tools\\Optional - Assessment Report - Template.docx"
                }
            else {
                Write-Host "This script requires specific Microsoft PowerPoint and Word templates, which are available in the Azure Proactive Resiliency Library. You can download the templates from this GitHub repository:"
                Write-Host "https://github.com/Azure/Azure-Proactive-Resiliency-Library-v2/tree/main/tools"
...
<#     if ($includeLow.IsPresent)
    ...
    #Call the functions
    $Global:LogFile = ($(Get-Location).Path + '\wara_reports_generator.log')
    ...
    $Global:PPTFinalFile = ($(Get-Location).path + '\Executive Summary Presentation - ' + $CustomerName + ' - ' + (get-date -Format "yyyy-MM-dd-HH-mm") + '.pptx')
    if ($WordTemplateFile)
        {
        ...
        Write-Host " "
        $Global:WordFinalFile = ($(Get-Location).path + '\Assessment Report - ' + $CustomerName + ' - ' + (get-date -Format "yyyy-MM-dd-HH-mm") + '.docx')
        }
    else
        {
        Write-Host "PowerPoint" -ForegroundColor DarkRed -NoNewline
...
<#     if($GenerateCSV.IsPresent)
    {
        ...
        $CSVFile = ($(Get-Location).path + '\Impacted Resources and Recommendations Template ' + (get-date -Format "yyyy-MM-dd-HH-mm") + '.csv')
